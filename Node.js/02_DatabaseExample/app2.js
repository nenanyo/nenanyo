...
router.route('/process/login').post(function(req, res) {
	...
	if (connection) {
		authUser(connection, paramId, paramPassword, function(err, docs) {
			if (err) { throw err; }
...
router.route('/process/adduser').post(function(req,res) {
    ...
    //데이터베이스 객체가 초기화된 경우, addUser 함수 호출 사용자 추가
    if (connection) {
        addUser(connection, paramId, paramPassword, paramName, function(err, result) {
            if (err) {throw err;}
...
var addUser = function(connection, id, password, name, callback) {
    ...
    //Mysql 쿼리 실행
    var sql = 'INSERT INTO user (id,pw,name) VALUES (?,?,?)';
